...
func TestMSH(t *testing.T) {
	...
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(MSHstringSource, delimeter)
	if err != nil {
		t.Errorf("TestMSH returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	MSHstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestPID(t *testing.T) {
	...
	delimeter.Childlevels = "^"
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(PIDstringSource, delimeter)
	if err != nil {
		t.Errorf("TestPID returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	PIDstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestORC(t *testing.T) {
	...
	delimeter.Childlevels = "^"
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(ORCstringSource, delimeter)
	if err != nil {
		t.Errorf("TestORC returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	ORCstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestOBR(t *testing.T) {
	...
	delimeter.Childlevels = "^"
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(OBRstringSource, delimeter)
	if err != nil {
		t.Errorf("TestOBR returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	OBRstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestOBX(t *testing.T) {
	...
	delimeter.Childlevels = "^"
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(OBXstringSource, delimeter)
	if err != nil {
		t.Errorf("TestOBX returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	OBXstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestTCD(t *testing.T) {
	...
	delimeter.Childlevels = "^"
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(TCDstringSource, delimeter)
	if err != nil {
		t.Errorf("TestTCD returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	TCDstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestNTE(t *testing.T) {
	...
	delimeter.Childlevels = "^"
	delimeter.CrLf = "\r"
	messages, err := gofor7.GetMessagesFromString(NTEstringSource, delimeter)
	if err != nil {
		t.Errorf("TestNTE returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	NTEstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
...
func TestComplett(t *testing.T) {
	...
	ComplettstringSource = fmt.Sprintf("%s%s%s", ComplettstringSource, delimeter.CrLf, ORCstringSource)
	messages, err := gofor7.GetMessagesFromString(ComplettstringSource, delimeter)
	if err != nil {
		t.Errorf("TestPID returned err: %v", err)
	}
	hl7messageSource, err := gofor7.GetHL7MessageFromMessages(messages)
	ComplettstringCreated, err := gofor7.GetStringFromHL7Messages(hl7messageSource, delimeter)
